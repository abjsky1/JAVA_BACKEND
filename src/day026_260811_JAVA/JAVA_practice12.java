...
public class JAVA_practice12 {
    ...
    public static void main(String[] args) {
        ...
        // tri 의 타입은 Triangle 인데 Figure 생성될 때부터 클래스에 상속되어있기 때문에 가능하다.
        ...
        if(vehicle instanceof Bus){System.out.println("Bus 타입이 맞습니다.");}

        vehicle = (Bus)vehicle;

    //  vehicle.checkFare();    error ㅠ

        Bus bus = (Bus)vehicle;

        bus.checkFare();

        ((Bus)vehicle).checkFare();  // 이것도 가능
        ...
        Beverage[] be = new Beverage[2];

        be[0] = new Coke();

        be[1] = new Coffee();

        for(Beverage beverage : be){beverage.drink();}
        ...
        */
        ...
        System.out.println( lap instanceof Electronic );    // true
        System.out.println( lap instanceof Device );        // true
