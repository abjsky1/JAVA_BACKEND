        ...
        /*
            [ 메소드 / 함수 ]
                ...
                3. 선언 :
                    ...
                    1) 접근제한자 반환타입 메소명 ( 타입 매개변수 ){ 실행문; return 반환값; }
                ...
                5. 지역변수 :
                    ...
                    - if / for / 메소드 { } 안에 매개변수/선언된변수 는 { } 밖에서 호출 불가능


            [ 접근제한자 ] *공부 필요*

                1. 서로 다른 클래스간 접근 여부 , 캡슐화

                2. 종류 : 

                    1) public       : 프로젝트 안에 어디에서든 호출/사용 가능
                    2) private      : 현재 클래스 안에 호출/사용 가능
                    3) (default)    : 같은 패키지 안에 호출/사용 가능
                    4) protected    : 같은 패키지 안에 호출/사용 가능.  단, 상속 관계 예외
                    
                3. 사용처 :

                    - 클래스 , 변수 , 생성자 , 메소드 등등

                * 관례적으로 멤버변수는 private 한다.
            ...
            [ 주요 키워드 ]
                ...
                1. static  :  
                    ...
                    - 인스턴스 없이 클래스 안에 변수와 메소드 사용 가능
                    - 프로그램 시작시 메모리가 할당되고, 프로그램 종료시 메모리가 삭제됨
                    - 멤버변수 또는 메소드 사용
                ...
                2. final  :  
                    ...
                    - 초기화 필수
                    - 멤버변수 또는 메소드 사용
                    - public static final = 초기값 , 상수(const)

                3. setter and getter  :  멤버변수가 private 인 경우 간접접근 제공하는 함수

                    - set멤버변수명( )  :  setter - 저장
                    - get멤버변수명( )  :  getter - 호출
                    - 클래스 안에 우클릭 -> 소스 작업 ->

                4. toString( )  :  인스턴스 호출시 주소값 대신 문자열 반환하는 메소드/함수

            
                5. 패키지  :  해당 클래스가 위치한 경로/위치

                6. import  :  
                
                    - 다른 클래스를 호출한 경우  .java 파일 상단에 호출한 클래스의 패키지/경로 표시

                    - 단, java.lang 패키지 ( System , String , Math ) 제외한 다른 클래스인 경우
...
class Member{
    ...
    private String id;  // 외부 클래스에서 접근 불가능한 멤버변수 - private
