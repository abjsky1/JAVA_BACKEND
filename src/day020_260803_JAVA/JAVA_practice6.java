...
public class JAVA_practice6 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("합계 : "+sum1+" , 평균 : "+(float)sum1/scores1.length);
        ...
        System.out.printf("합계 : %d , 평균 : %f", sum1, (float)sum1/scores1.length);
        ...
        for( int score : scores2 ){ if (score==100) { System.out.println("만점자"); } }
        ...
        for(int i = 0 ; i <= bloodTypes.length-1 ; i++){
            if(bloodTypes[i].equals("A")){ c++; }
        }
        System.out.println(c+"명");

        c = 0;
        for(String str : bloodTypes ){ if( str.equals("A")){c++;}}
        System.out.println(c+"명");
        ...
        a = 0;
        for(int max : numbers2){if(max > a){a = max;} System.out.println(a);}
        ...
        for(int i = 0 ; i <= products.length-1 ; i++){
            if(buy.equals(products[i])){
                int hm = scan.nextInt();
                if(hm > stock[i]){System.out.println("재고가 부족합니다."); break;}
                else { 
                    stock[i] -= hm;
                    System.out.println("구매완료"); break;
                }
            }
            else{System.out.println("없는 제품명입니다.");}
        }
        ...
        for(int i = 0 ; i <= carNumbers.length-1 ; i++){
            int fee = 0;
            if(usageMinutes[i] <= 30){fee = 1000;}
            else{ fee = 1000 + ((usageMinutes[i]-30) / 10)*500; }

            if (fee > 20000) {fee = 20000;}
            
            System.out.printf("\n%s: %d분 주차, 최종 요금: %d원", carNumbers[i] , usageMinutes[i] , fee);
        }


        for(int i = 0 ; i <= carNumbers.length-1 ; i++){
            int fee = 1000;
            if(usageMinutes[i] > 30){ fee = 1000 + ((usageMinutes[i]-30) / 10)*500; }
